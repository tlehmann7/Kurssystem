<?php
	...
	if($_SESSION['auth'])
	{
		...
		if(!$ref->connect_error)
		{
			...
			{	
				$query_string = "SELECT * FROM ".$db_table_courses." WHERE ID = ".$cid.";";
				$coursedata = $ref->query($query_string)->fetch_array(MYSQLI_ASSOC)[0];
				if($_SESSION['user'] == $coursedata['owner'])
				{
					if(isset($pid))
					{
						...
						$hline1 = csvquote($coursedata['name']." - ".$pname).$csvNewline;
						$hline2 = "Vorname".$csvSep."Nachname".$csvSep."Abiturjahrgang".$csvSep."Klasse".$csvNewline;
						fwrite($out, $hline1.$hline2);
						for($i = 0; $i < count($presults); $i++)
						{
							$line = csvquote($presults[$i]['vname']).$csvSep.csvquote($presults[$i]['nname']).$csvSep.csvquote($presults[$i]['alevel']).$csvSep.csvquote($presults[$i]['class']).$csvNewline;
							fwrite($out, $line);
						}
						fclose($out);
					}
					else if(!isset($pid))
					{
						...
						header("Expires: 0");
						$out = fopen("php://output", "w");
						$hline = $coursedata['name'].$csvNewline;
						fwrite($out, $hline);
						for($i = 0; $i < count(); $i++)
						{
			...
			{
				$query_string = "SELECT * FROM ".$db_table_courses." WHERE ID = ".$cid.";";
				$coursedata = $ref->query($query_string)->fetch_array(MYSQLI_ASSOC);
				if(isset($pid))
				{
					$query_string = "SELECT * FROM ".$cid."_".$pid.$projectSignUpPostfix.";";
					$presults = $ref->query($query_string)->fetch_all(MYSQLI_ASSOC);
					...
					header("Expires: 0");
					$out = fopen("php://output", "w");
					$hline1 = $coursedata['name']." - ".$pname.$csvNewline;
					$hline2 = "Vorname".$csvSep."Nachname".$csvSep."Abiturjahrgang".$csvSep."Klasse".$csvNewline;
					fwrite($out, $hline1.$hline2);
					for($i = 0; $i < count($presults); $i++)
					{
				...
				if($canDownload)
				{
					if(!isset($pid))
					{
						...
						for($i = 0; $i < count($projInfos); $i++)
						{
							...
							$hline = csvquote("Vorname").$csvSep.csvquote("Nachname").$csvSep.csvquote("Abiturjahrgang").$csvSep.csvquote("Klasse").$csvNewline;
							fwrite($out, $hline);
							
							for($p = 0; $p < count($presults); $p++)
							{					
								$line = csvquote($presults[$p]['vname']).$csvSep.csvquote($presults[$p]['nname']).$csvSep.csvquote($presults[$p]['alevel']).$csvSep.csvquote($presults[$p]['class']).$csvNewline;
					...
					else
					{
						...
						$out = fopen("php://output", "w");
						$hline1 = csvquote($coursedata['name']." - ".$pname).$csvNewline;
						$hline2 = csvquote("Vorname").$csvSep.csvquote("Nachname").$csvSep.csvquote("Abiturjahrgang").$csvSep.csvquote("Klasse").$csvNewline;
						fwrite($out, $hline1.$hline2);
						for($i = 0; $i < count($presults); $i++)
						{
							$line = csvquote($presults[$i]['vname']).$csvSep.csvquote($presults[$i]['nname']).$csvSep.csvquote($presults[$i]['alevel']).$csvSep.csvquote($presults[$i]['class']).$csvNewline;
